# mini-proyectos-practica
Repository to share my Python mini‑projects and practice exercises. I use this space to document my learning progress in programming and cybersecurity, and to receive feedback for continuous improvement.

## 👨‍💻 About Me
My name is **Fermín Longoni**, an Argentinian Software Development student.  
I’m 18 years old and deeply interested in learning about **Python** and **Cybersecurity**.  
I enjoy building small projects, experimenting with code, and improving my skills.

## 📚 Goals
- Strengthen my Python fundamentals through practical exercises.
- Explore cybersecurity concepts with platforms like TryHackMe.
- Document my progress and share it openly.
- Receive feedback to continuously improve my coding style and problem-solving approach.

## 🚀 What You’ll Find Here
- **Python mini-projects:** small utilities, math functions, and interactive exercises.
- **Practice scripts:** experiments with validation, error handling, and logic building.
- **Learning notes:** reflections and lessons learned from each project.

## 🤝 Contributions
Feedback and suggestions are welcome!  
This repository is part of my learning journey, and I’m open to collaboration and advice from the community.
